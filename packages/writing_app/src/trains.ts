...
import trainBrakeVanSprite from "./assets/trains/brake_van.png";
import trainCarriage1Sprite from "./assets/trains/carriage_1.png";
import trainCarriage2Sprite from "./assets/trains/carriage_2.png";
import trainEngineSprite from "./assets/trains/engine.png";
import trainBackgroundImage from "./assets/trains/train_bg.png";
...
const registerTrainServiceWorker = () => {
  if (!import.meta.env.PROD || !("serviceWorker" in navigator)) {
    return;
  }

  const scope = `${import.meta.env.BASE_URL}trains/`;
  void navigator.serviceWorker.register(`${scope}sw.js`, { scope }).catch((error: unknown) => {
    console.error("Failed to register trains service worker.", error);
  });
};
...
const SNAKE_SEGMENT_SPACING = 132;
...
const TRAIN_ENTRY_MARGIN = 320;
const TRAIN_ENTRY_SPEED = 420;
...
if (!app) {
  throw new Error("Missing #app element for trains app.");
}

initializeAnalytics();
registerTrainServiceWorker();
      ...
      <section class="writing-app__board">
        <header class="writing-app__topbar">
          <div class="writing-app__title">
            <p class="writing-app__eyebrow">Drive the train around the letters.</p>
        ...
        <svg
          ...
          aria-label="Handwriting train tracing area"
        ></svg>

        <div class="writing-app__overlay" id="success-overlay" hidden>
          <div class="writing-app__success-card">
            <p class="writing-app__success-eyebrow">Train arrived!</p>
...
app.style.setProperty("--snake-board-image", `url("${trainBackgroundImage}")`);
...
if (
  ...
  throw new Error("Missing elements for trains app.");
}
...
let displayPreparedPath: PreparedTracingPath | null = null;
...
let trainEntryAnimationFrameId: number | null = null;
...
let isTrainEntering = false;
let snakeExitBodyCount = 0;
let trainEntryStopDistance = 0;
let trainExitStartDistance = 0;
...
const buildTrainLayouts = (
  word: string
): {
  displayPath: WritingPath;
  width: number;
  height: number;
  offsetY: number;
} => {
  const rawDisplayPath = buildHandwritingPath(word, {
    style: "cursive",
  ...
  if (rawDisplayPath.strokes.length === 0) {
    throw new Error(`No drawable strokes found for "${word}".`);
  }

  const paddingX = 180;
  const paddingY = 150;
  const width = Math.ceil(rawDisplayPath.bounds.maxX - rawDisplayPath.bounds.minX + paddingX * 2);
  const height = Math.ceil(rawDisplayPath.bounds.maxY - rawDisplayPath.bounds.minY + paddingY * 2);
  const offsetX = paddingX - rawDisplayPath.bounds.minX;
  const offsetY = paddingY - rawDisplayPath.bounds.minY;

  return {
    displayPath: shiftWritingPath(rawDisplayPath, offsetX, offsetY),
    width,
    height,
    offsetY
  };
...
const getPreparedPathTotalLength = (preparedPath: PreparedTracingPath): number =>
  preparedPath.strokes.reduce((total, stroke) => total + stroke.totalLength, 0);

const samplePreparedPathPoseAtDistance = (
  ...
  for (let strokeIndex = 0; strokeIndex < preparedPath.strokes.length; strokeIndex += 1) {
    ...
    if (remaining > stroke.totalLength && strokeIndex < preparedPath.strokes.length - 1) {
      remaining -= stroke.totalLength;
      continue;
    }

    const distanceAlongStroke = Math.max(0, Math.min(remaining, stroke.totalLength));
    const point = interpolateSamplePoint(stroke.samples, distanceAlongStroke);
    const nextPoint = interpolateSamplePoint(
      stroke.samples,
      Math.min(stroke.totalLength, distanceAlongStroke + 2)
    );
    const previousPoint = interpolateSamplePoint(stroke.samples, Math.max(0, distanceAlongStroke - 2));
  ...
  const lastStroke = preparedPath.strokes[preparedPath.strokes.length - 1];
  const lastSample = lastStroke?.samples[lastStroke.samples.length - 1];
  return {
...
const renderDeferredSnake = (
  ...
  headImageEl.setAttribute("href", options.headHref ?? trainEngineSprite);
  setSpritePose(
    headGroupEl,
    headImageEl,
    {
...
const createDeferredSnakeMarkup = (attributes: string): string => `
  <g ${attributes}>
    <g class="writing-app__deferred-head-part" data-deferred-part="tail">
      <image href="${trainBrakeVanSprite}" preserveAspectRatio="none"></image>
    </g>
    <g class="writing-app__deferred-head-part" data-deferred-part="body">
      <image href="${trainCarriage1Sprite}" preserveAspectRatio="none"></image>
    </g>
    <g class="writing-app__deferred-head-part" data-deferred-part="head">
      <image href="${trainEngineSprite}" preserveAspectRatio="none"></image>
    </g>
  </g>
`;
...
const setSpritePose = (
  ...
  const shouldMirror = pose.angle > 90 || pose.angle < -90;
  const normalizedRotation = normalizeAngleDegrees(
    shouldMirror ? pose.angle - 180 : pose.angle
  );
  const adjustedRotationOffset = shouldMirror ? -rotationOffset : rotationOffset;
  ...
  groupEl.setAttribute(
    ...
    `translate(${pose.x.toFixed(2)} ${pose.y.toFixed(2)}) rotate(${(
      normalizedRotation + adjustedRotationOffset
    ).toFixed(2)}) scale(${shouldMirror ? -1 : 1} 1)`
  );
...
const renderSnake = (now = performance.now()) => {
  ...
  snakeHeadImageEl.setAttribute("href", trainEngineSprite);
  setSpritePose(
    snakeHeadEl,
    snakeHeadImageEl,
    {
...
const cancelTrainEntryAnimation = () => {
  if (trainEntryAnimationFrameId !== null) {
    cancelAnimationFrame(trainEntryAnimationFrameId);
    trainEntryAnimationFrameId = null;
  }
  isTrainEntering = false;
};

const startTrainEntry = () => {
  const session = tracingSession;
  const preparedDisplayPath = displayPreparedPath;
  const state = session?.getState();
  if (!state || !session || !preparedDisplayPath) {
    hideSnakeSprites();
    return;
  }

  cancelTrainEntryAnimation();
  const displayStartPose = samplePreparedPathPoseAtDistance(preparedDisplayPath, 0);
  const entryStartPoint = {
    x: Math.min(-TRAIN_ENTRY_MARGIN, displayStartPose.point.x - TRAIN_ENTRY_MARGIN),
    y: displayStartPose.point.y
  };
  const approachDistance = Math.max(0, displayStartPose.point.x - entryStartPoint.x);
  const totalTravelDistance = approachDistance + trainEntryStopDistance;
  const entryStartTime = performance.now();

  resetSnakeTrail(entryStartPoint, { x: 1, y: 0 }, true);
  isTrainEntering = true;
  activePointerId = null;
  activePointerPosition = null;
  session.reset();

  const tick = (now: number) => {
    const elapsedSeconds = Math.max(0, now - entryStartTime) / 1000;
    const travelled = Math.min(totalTravelDistance, elapsedSeconds * TRAIN_ENTRY_SPEED);

    if (travelled <= approachDistance) {
      appendSnakePose(
        {
          x: entryStartPoint.x + travelled,
          y: entryStartPoint.y
        },
        { x: 1, y: 0 },
        true
      ...
      const pathPose = samplePreparedPathPoseAtDistance(preparedDisplayPath, travelled - approachDistance);
      appendSnakePose(pathPose.point, pathPose.tangent, true);
    }
    ...
    if (travelled >= totalTravelDistance) {
      ...
      appendSnakePose(resetState.cursorPoint, resetState.cursorTangent, true);
      requestTraceRender();
      return;
    }

    trainEntryAnimationFrameId = requestAnimationFrame(tick);
  };
...
const startSnakeExit = (point: Point, tangent: Point) => {
  ...
  const displayTotalDistance = displayPreparedPath ? getPreparedPathTotalLength(displayPreparedPath) : 0;
  const leadOutDistance = Math.max(0, displayTotalDistance - trainExitStartDistance);
  const leadOutEndPose =
    displayPreparedPath && displayTotalDistance > 0
      ? samplePreparedPathPoseAtDistance(displayPreparedPath, displayTotalDistance)
      : { point, tangent: direction };
  const offscreenTravelDistance = getExitTravelDistance(
    leadOutEndPose.point,
    leadOutEndPose.tangent,
    snakeExitBodyCount
  );
  const totalTravelDistance = leadOutDistance + offscreenTravelDistance;
  ...
  const tick = (now: number) => {
    ...
    if (displayPreparedPath && travelled <= leadOutDistance) {
      const leadOutPose = samplePreparedPathPoseAtDistance(
        displayPreparedPath,
        Math.min(displayTotalDistance, trainExitStartDistance + travelled)
      );
      appendSnakePose(leadOutPose.point, leadOutPose.tangent, true);
    } else {
      const offscreenTravelled = Math.max(0, travelled - leadOutDistance);
      appendSnakePose(
        {
          x: leadOutEndPose.point.x + leadOutEndPose.tangent.x * offscreenTravelled,
          y: leadOutEndPose.point.y + leadOutEndPose.tangent.y * offscreenTravelled
        },
        leadOutEndPose.tangent,
        true
      );
...
const renderTraceFrame = () => {
  ...
  if (
    ...
    !isSnakeExitComplete &&
    !isTrainEntering &&
    hasOnlyDeferredStrokesRemaining(state)
  ) {
  ...
  if (state.status === "complete") {
    if (!isDemoPlaying && !isSnakeSlithering && !isSnakeExitComplete && !isTrainEntering) {
      const exitPose = getSnakeExitPose(state);
      startSnakeExit(exitPose.point, exitPose.tangent);
    }
...
const playDemo = () => {
  ...
  const player = new AnimationPlayer(currentPath, {
    speed: 0.55 * SHOW_ME_SPEED_MULTIPLIER,
    penUpSpeed: 1.05 * SHOW_ME_SPEED_MULTIPLIER,
    deferredDelayMs: 150
  });
  ...
  showMeButton.textContent = "Demo...";
  syncFruitDisplay();
  const initialFrame = player.getFrame(0);
  const initialDirection =
    Math.hypot(initialFrame.velocity.x, initialFrame.velocity.y) > 0.001
      ? normalizeVector(initialFrame.velocity)
      : { x: 1, y: 0 };
  resetSnakeTrail(initialFrame.point, initialDirection, true);
  renderSnake();
  ...
  const tick = (now: number) => {
    ...
    const frameDirection =
      Math.hypot(frame.velocity.x, frame.velocity.y) > 0.001
        ? normalizeVector(frame.velocity)
        : pointFromAngle(snakeHeadAngle);
    appendSnakePose(frame.point, frameDirection, true);
...
const setupScene = (
  ...
  displayPreparedPath = compileTracingPath(fullPath);
  const preparedPath = compileTracingPath(fullPath);
  preparedTracingPath = preparedPath;
  drawablePathStrokes = fullPath.strokes.filter((stroke) => stroke.type !== "lift");
  ...
  const snakeBodyMarkup = Array.from({ length: MAX_SNAKE_BODY_SEGMENTS }, (_, index) => {
    ...
    const bodySpriteHref = bodyIndex % 2 === 0 ? trainCarriage1Sprite : trainCarriage2Sprite;
    return `
      <g
  ...
  traceSvg.innerHTML = `
    ...
    <line
      ...
      x1="0"
      y1="${fullPath.guides.xHeight + offsetY}"
      x2="${width}"
      y2="${fullPath.guides.xHeight + offsetY}"
    ></line>
    <line
      class="writing-app__guide writing-app__guide--baseline"
      x1="0"
      y1="${fullPath.guides.baseline + offsetY}"
      x2="${width}"
      y2="${fullPath.guides.baseline + offsetY}"
    ></line>
    ${backgroundPaths}
    ${tracePaths}
    ${demoPaths}
  ...
  trainEntryStopDistance = 0;
  trainExitStartDistance = currentPathLength;
...
const renderWord = (word: string, wordIndex = -1) => {
  ...
  const layout = buildTrainLayouts(word);
  currentWord = word;
...
const onPointerDown = (event: PointerEvent) => {
  if (isDemoPlaying || isTrainEntering || !tracingSession || activePointerId !== null) {
    return;
  }
...
const onPointerMove = (event: PointerEvent) => {
  if (isDemoPlaying || isTrainEntering || !tracingSession || event.pointerId !== activePointerId) {
    return;
  }
