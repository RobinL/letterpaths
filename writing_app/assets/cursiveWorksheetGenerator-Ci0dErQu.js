  ...
  <div class="worksheet-app">
    <aside class="worksheet-app__controls" aria-label="Worksheet controls">
      <div class="worksheet-app__controls-inner">
        ...
        ${p({id:"preview-zoom-slider",label:"Preview zoom",value:z,min:50,max:200,step:5,valueId:"preview-zoom-value"})}

        ${p({id:"practice-size-slider",label:"Practice size",value:se,min:14,max:38,step:1,valueId:"practice-size-value"})}

        ${p({id:"practice-repeat-slider",label:"Practice repeats",value:de,min:1,max:6,step:1,valueId:"practice-repeat-value"})}

        ${p({id:"stroke-width-slider",label:"Main stroke thickness",value:le,min:20,max:90,step:2,valueId:"stroke-width-value"})}
...
`;const D=document.querySelector("#worksheet-text-input"),$=document.querySelector("#preview-zoom-slider"),y=document.querySelector("#practice-size-slider"),A=document.querySelector("#practice-repeat-slider"),G=document.querySelector("#stroke-width-slider"),pe=document.querySelector("#print-worksheet-button"),C=document.querySelector("#download-png-button"),we=document.querySelector("#worksheet-page-frame"),g=document.querySelector("#worksheet-page"),v=document.querySelector("#worksheet-status");if(!D||!$||!y||!A||!G||!pe||!C||!we||!g||!v)throw new Error("Missing elements for cursive worksheet generator.");const V=Array.from(document.querySelectorAll("[data-global-setting]")),X=Array.from(document.querySelectorAll("[data-scope][data-setting]")),ge=Array.from(document.querySelectorAll("[data-scope][data-annotation-kind]"));function p({id:e,label:r,value:t,min:a,max:n,step:o,valueId:s=`${e}-value`,attrs:d=""}){return`
  ...
  `}function tt(){return`
    <details class="worksheet-app__details">
      ...
      <div class="worksheet-app__details-body">
        ${p({id:"target-bend-rate-slider",label:"Target maximum bend rate",value:k.targetBendRate,min:0,max:60,step:1,valueId:"target-bend-rate-value",attrs:'data-global-setting="targetBendRate"'})}
        ${p({id:"min-sidebearing-gap-slider",label:"Minimum sidebearing gap",value:k.minSidebearingGap,min:-300,max:200,step:5,valueId:"min-sidebearing-gap-value",attrs:'data-global-setting="minSidebearingGap"'})}
        ${p({id:"bend-search-min-sidebearing-gap-slider",label:"Search minimum sidebearing gap",value:k.bendSearchMinSidebearingGap,min:-300,max:200,step:5,valueId:"bend-search-min-sidebearing-gap-value",attrs:'data-global-setting="bendSearchMinSidebearingGap"'})}
        ${p({id:"bend-search-max-sidebearing-gap-slider",label:"Search maximum sidebearing gap",value:k.bendSearchMaxSidebearingGap,min:-100,max:300,step:5,valueId:"bend-search-max-sidebearing-gap-value",attrs:'data-global-setting="bendSearchMaxSidebearingGap"'})}
        ${p({id:"exit-handle-scale-slider",label:"p0-p1 handle scale",value:k.exitHandleScale,min:0,max:2,step:.05,valueId:"exit-handle-scale-value",attrs:'data-global-setting="exitHandleScale"'})}
        ${p({id:"entry-handle-scale-slider",label:"p2-p3 handle scale",value:k.entryHandleScale,min:0,max:2,step:.05,valueId:"entry-handle-scale-value",attrs:'data-global-setting="entryHandleScale"'})}
  ...
  `}function rt(){return`
    <details class="worksheet-app__details">
      ...
      <div class="worksheet-app__details-body">
        ${p({id:"gridline-stroke-width-slider",label:"Gridline thickness",value:ce,min:.5,max:8,step:.5,valueId:"gridline-stroke-width-value",attrs:'data-global-setting="gridlineStrokeWidth"'})}
  ...
  ${i.showAscenderGuide?`
    ...
    ></line>
  `:""}
`,$e=(e,r,t)=>{const n=e.path.strokes.filter(s=>s.type!=="lift").map(s=>`<path class="worksheet-word__stroke" d="${Ae(s.curves)}"></path>`).join(""),o=Me(e.path,r,t);return`
    ${n}
    ${o}
  ...
  `},ht=e=>{const r=e.path.bounds.maxX-e.path.bounds.minX,t=e.path.bounds.minX;return r+t},ut=(e,r,t,a,n)=>{const o=ht(e),s=e.width+o*(a-1),d=$e(e,r,t),m=`practice-word-${n}`,b=Array.from({length:a},(H,R)=>{const xe=R*o;return`<use href="#${m}" x="${xe}" y="0"></use>`}).join("");return`
    <svg
  ...
  `},w=()=>{if(i={...i,text:Y(D.value),practiceRowHeightMm:Number(y.value),practiceRepeatCount:Number(A.value),strokeWidth:Number(G.value)},q(),be(),i.text.length===0){g.innerHTML=`
    ...
    `,v.textContent="";return}const e={joinSpacing:i.joinSpacing,keepInitialLeadIn:i.keepInitialLeadIn,keepFinalLeadOut:i.keepFinalLeadOut};let r;try{r=Le(i.text,e)}catch{g.innerHTML=`
      <div class="worksheet-page__empty">Use supported cursive letters and spaces.</div>
    `,v.textContent="This text could not be drawn.";return}const t=ye(r.path),a=ct(r,t,i.top,"worksheet-word worksheet-word--top",`${i.text} with formation annotations`),n=ot(),o=Array.from({length:n},(s,d)=>ut(r,t,i.practice,i.practiceRepeatCount,d)).join("");g.style.setProperty("--practice-row-height",`${i.practiceRowHeightMm}mm`),g.innerHTML=`
