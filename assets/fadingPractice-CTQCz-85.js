import"./Z-upper-print-bezier-o7EP_mHt.js";import{c as Pe}from"./style-DEZiQ06N.js";import"./joiner-C20HBd4f.js";import{b as Oe,a as Ne}from"./shared-Dlc__k-V.js";import{s as We,D as xe,b as Re}from"./worksheet-preview-pan-zoom-bTqdVrfd.js";import"./annotations-GdrJJm6N.js";const V="practice",Fe="cursive",X=100,Y=35,K=200,f=5,Me=20,j=12,J=2,Q=0,ee=480,te=1,re=140,ae=50,oe=10,ie=20,se=.8,ne="#9bb7d8",le="#d5dbe2",He="#83b0dd",Ue=96,de=320,ce=13,Ze=53,Be=53,pe=26,ue=5.6,ze=ce*2,qe=0,Ve="#3f454b",he="#ffffff",Xe=224,Ye=.63,Ke=.66,je=["text","word","style","previewZoom","rowHeight","rowGap","letterSpacing","wordSpacing","repeatCount","repeatGap","strokeWidth","fadeRows","initialTraceOpacity","showBaselineGuide","showXHeightGuide","showAscenderGuide","showDescenderGuide","guideStrokeWidth","guideColor","traceColor","keepInitialLeadIn","keepFinalLeadOut","includeNameDate"],W=document.querySelector("#app");if(!W)throw new Error("Missing #app element for fading handwriting practice.");document.body.classList.add("worksheet-body");W.classList.add("worksheet-root");const x=()=>({text:V,style:Fe,previewZoom:X,rowHeightMm:j,rowGapMm:J,letterSpacing:Q,wordSpacing:ee,repeatCount:te,repeatGap:re,strokeWidth:ae,fadeRows:oe,initialTraceOpacity:ie,showBaselineGuide:!0,showXHeightGuide:!0,showAscenderGuide:!0,showDescenderGuide:!0,guideStrokeWidth:se,guideColor:ne,traceColor:le,keepInitialLeadIn:!0,keepFinalLeadOut:!0,includeNameDate:!1}),Je=t=>({"directional-dash":t["directional-dash"],"turning-point":t["turning-point"],"start-arrow":t["start-arrow"],"draw-order-number":t["draw-order-number"],"midpoint-arrow":t["midpoint-arrow"]}),Qe=()=>({directionalDashSpacing:Ue,midpointDensity:de,turnRadius:ce,uTurnLength:Ze,arrowLength:Be,arrowHeadSize:pe,arrowStrokeWidth:ue,numberSize:ze,numberPathOffset:qe,numberColor:Ve,offsetArrowLanes:!0,alwaysOffsetArrowLanes:!1,visibility:Je(xe),arrowColor:he,strokeColor:He}),et=(t,r)=>({...t,...r,visibility:r.visibility?{...t.visibility,...r.visibility}:t.visibility}),C=et(Qe(),{directionalDashSpacing:152,midpointDensity:de,turnRadius:48,uTurnLength:52,arrowLength:149,arrowHeadSize:pe,arrowStrokeWidth:5.5,numberSize:64,numberPathOffset:-77,offsetArrowLanes:!1,visibility:{"directional-dash":!0,"turning-point":!1,"start-arrow":!1,"draw-order-number":!0,"midpoint-arrow":!1}}),l=x();let e=x(),D=!1;W.innerHTML=`
    ...
    <aside class="worksheet-app__controls" aria-label="Worksheet controls">
      <div class="worksheet-app__controls-inner">
        ...
        <div class="worksheet-app__standalone-spacing-controls" id="standalone-spacing-controls" hidden>
          ${p({id:"letter-spacing-slider",label:"Letter spacing",value:Q,min:-40,max:280,step:10,valueId:"letter-spacing-value"})}
          ${p({id:"word-spacing-slider",label:"Space width",value:ee,min:180,max:960,step:20,valueId:"word-spacing-value"})}
        </div>

        ${p({id:"row-height-slider",label:"Line height",value:j,min:8,max:24,step:1,valueId:"row-height-value"})}

        ${p({id:"row-gap-slider",label:"Line spacing",value:J,min:0,max:12,step:1,valueId:"row-gap-value"})}

        ${p({id:"repeat-count-slider",label:"Words per line",value:te,min:1,max:8,step:1,valueId:"repeat-count-value"})}

        ${p({id:"repeat-gap-slider",label:"Word spacing",value:re,min:0,max:420,step:10,valueId:"repeat-gap-value"})}

        ${p({id:"fade-rows-slider",label:"Fading trace rows",value:oe,min:2,max:20,step:1,valueId:"fade-rows-value"})}

        ${p({id:"initial-trace-opacity-slider",label:"Initial trace darkness",value:ie,min:1,max:100,step:1,valueId:"initial-trace-opacity-value"})}

        ${p({id:"stroke-width-slider",label:"Stroke thickness",value:ae,min:20,max:90,step:2,valueId:"stroke-width-value"})}
        ...
        <details class="worksheet-app__details">
          ...
          <div class="worksheet-app__details-body">
            ${p({id:"guide-stroke-width-slider",label:"Line thickness",value:se,min:.4,max:3,step:.1,valueId:"guide-stroke-width-value",attrs:'data-global-setting="guideStrokeWidth"'})}
            ${B("guide-color-picker","guideColor","Line colour",ne)}
            ${B("trace-color-picker","traceColor","Trace colour",le)}
...
`;const P=document.querySelector("#worksheet-text-input"),O=document.querySelector("#worksheet-style-select"),we=document.querySelector("#standalone-spacing-controls"),me=document.querySelector("#preview-zoom-in-button"),ge=document.querySelector("#preview-zoom-out-button"),ke=document.querySelector("#worksheet-preview-viewport"),v=document.querySelector("#row-height-slider"),_=document.querySelector("#row-gap-slider"),S=document.querySelector("#letter-spacing-slider"),b=document.querySelector("#word-spacing-slider"),L=document.querySelector("#repeat-count-slider"),$=document.querySelector("#repeat-gap-slider"),G=document.querySelector("#fade-rows-slider"),T=document.querySelector("#initial-trace-opacity-slider"),E=document.querySelector("#stroke-width-slider"),fe=document.querySelector("#print-worksheet-button"),w=document.querySelector("#worksheet-page-frame"),m=document.querySelector("#worksheet-page"),I=document.querySelector("#worksheet-status");if(!P||!O||!we||!me||!ge||!ke||!v||!_||!S||!b||!L||!$||!G||!T||!E||!fe||!w||!m||!I)throw new Error("Missing elements for fading handwriting practice.");const R=Array.from(document.querySelectorAll("[data-global-setting]"));function tt(){return`
  ...
  `}function p({id:t,label:r,value:a,min:i,max:s,step:o,valueId:n=`${t}-value`,attrs:y=""}){return`
...
`,st=t=>{const r=t.path.bounds.maxX-t.path.bounds.minX,a=t.path.bounds.minX;return r+a+e.repeatGap},nt=(t,r,a,i,s="xMidYMid meet",o=1,n="practice-word")=>{const y=st(t),U=t.width+y*(o-1),Z=H(t),Ce=o<=1?Z:`
        ...
        ${Array.from({length:o},(ut,Ie)=>{const De=Ie*y;return`<use href="#${n}" x="${De}" y="0"></use>`}).join("")}
      `;return`
    <svg
      class="${r}"
  ...
  `},lt=t=>{const a=e.style==="cursive"?Pe(t.path):null,i=a?it(t,a):H(t),s=a?C.strokeColor:e.traceColor,o=a?C.arrowColor:he,n=a?C.arrowStrokeWidth:ue;return`
    <svg
